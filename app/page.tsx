...
type GameState = 'setup' | 'names' | 'reveal' | 'playing' | 'voting' | 'results';
type Category = 'places' | 'food' | 'activities' | 'nature' | 'entertainment' | 'random';
...
export default function Home() {
  ...
  const categories: Record<Category, WordCategory> = {
    places: {
      ...
      words: ['Airport', 'Beach', 'Castle', 'Desert', 'Forest', 'Hospital', 'Library', 'Museum', 'Park', 'Prison', 'School', 'Space Station', 'Stadium', 'Subway', 'Zoo', 'Haunted House', 'Volcano', 'Underwater City', 'Cloud City', 'Moon Base']
    },
    food: {
      name: 'Food & Drinks',
      emoji: '🍕',
      gradient: 'from-orange-500 to-red-500',
      words: ['Pizza', 'Sushi', 'Burger', 'Ice Cream', 'Tacos', 'Pasta', 'Salad', 'Chocolate', 'Coffee', 'Smoothie', 'Ramen', 'Curry', 'BBQ', 'Sandwich', 'Soup', 'Cake', 'Steak', 'Seafood', 'Fruit Salad', 'Hot Dog']
    },
    activities: {
      name: 'Activities',
      emoji: '🎮',
      gradient: 'from-purple-500 to-pink-500',
      words: ['Gaming', 'Swimming', 'Dancing', 'Painting', 'Reading', 'Cooking', 'Hiking', 'Shopping', 'Sleeping', 'Working Out', 'Photography', 'Gardening', 'Singing', 'Writing', 'Coding', 'Meditation', 'Yoga', 'Skateboarding', 'Rock Climbing', 'Bungee Jumping']
    },
    nature: {
      name: 'Nature',
      emoji: '🌿',
      gradient: 'from-green-500 to-emerald-500',
      words: ['Ocean', 'Mountain', 'Rainforest', 'Desert', 'River', 'Waterfall', 'Cave', 'Island', 'Glacier', 'Canyon', 'Volcano', 'Coral Reef', 'Savanna', 'Tundra', 'Swamp', 'Prairie', 'Fjord', 'Geyser', 'Aurora', 'Rainbow']
    },
    entertainment: {
      name: 'Entertainment',
      emoji: '🎬',
      gradient: 'from-indigo-500 to-purple-500',
      words: ['Movie Theater', 'Concert', 'Video Game', 'TV Show', 'Podcast', 'Stand-up Comedy', 'Musical', 'Opera', 'Ballet', 'Magic Show', 'Circus', 'Festival', 'Theme Park', 'Escape Room', 'Karaoke', 'Board Game', 'Book Club', 'Art Gallery', 'Sports Game', 'Trivia Night']
    },
    random: {
      name: 'Mix It Up!',
      emoji: '🎲',
  ...
  const startGame = () => {
    ...
    if (savedNames) {
      ...
      if (parsedNames.length === playerCount && parsedNames.every((name: string) => name.trim() !== '')) {
        ...
        >
          {gameState === 'setup' && (
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl h-fit">
              ...
              >
                <h1 className="text-4xl sm:text-5xl font-bold mb-2 bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent" style={{ fontFamily: 'Space Grotesk' }}>
                  IMPOSTOR
                </h1>
                <p className="text-gray-500 text-sm sm:text-base">One of you doesn't belong...</p>
              </motion.div>
              ...
              <div className="space-y-4">
                ...
                >
                  <label className="text-gray-700 text-sm font-medium mb-2 block">
                    Players: <span className="text-xl font-bold text-gray-900">{playerCount}</span>
                  </label>
                  <input
                    type="range"
                    min="3"
                    ...
                    onChange={(e) => {
                      ...
                      if (playerNames.length > 0 && playerNames.length !== newCount) {
                        setPlayerNames([]);
                      }
                    }}
                ...
                >
                  <label className="text-gray-700 text-sm font-medium mb-2 block">
                    Timer: <span className="text-xl font-bold text-gray-900">{formatTime(gameTime)}</span>
                  </label>
                  <input
                    type="range"
                    min="60"
                ...
                {playerNames.length > 0 && playerNames.every(name => name.trim() !== '') && (
                  ...
                  >
                    <p className="text-blue-700 text-sm font-medium mb-2">Current Players:</p>
                    <div className="flex flex-wrap gap-2">
                      {playerNames.slice(0, playerCount).map((name, i) => (
                        <span key={i} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-lg text-xs font-medium">
                          {name}
                        </span>
                      ))}
                    </div>
            ...
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl h-fit">
              ...
              <div className="space-y-3 mb-6">
                {Array.from({ length: playerCount }, (_, i) => (
                  ...
                  >
                    <label className="text-gray-600 text-xs mb-1 block font-medium">Player {i + 1}</label>
                    <input
                      type="text"
                      value={playerNames[i] || ''}
                      onChange={(e) => {
              ...
              <motion.button
                ...
                disabled={playerNames.some(name => !name || name.trim() === '')}
            ...
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl h-fit">
              ...
              {!showWord ? (
                ...
                >
                  ...
                  <p className="text-gray-600 text-lg mb-2">Ready to see your word?</p>
                  <p className="text-gray-400 text-sm mb-6">Make sure others can't see!</p>
                ...
                >
                  ...
                  >
                    <p className="text-4xl sm:text-5xl font-bold mb-3" style={{ fontFamily: 'Space Grotesk' }}>
                      {currentPlayer === impostorIndex ? (
                        <span className="text-red-600">IMPOSTOR</span>
                      ) : (
                        <span className="text-blue-600">{word.toUpperCase()}</span>
                      )}
                    </p>
                    {currentPlayer === impostorIndex && (
                      <motion.p 
                  ...
                  >
                    {currentPlayer < playerCount - 1 ? 'Next Player' : 'Start Round'}
                  </motion.button>
                </motion.div>
              )}
            </div>
          ...
          {gameState === 'playing' && (
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl text-center h-fit">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800" style={{ fontFamily: 'Space Grotesk' }}>
                GAME ON!
              </h2>
              ...
              >
                ...
                <p className="text-gray-700 text-base mb-1 font-medium">Take turns giving clues!</p>
                <p className="text-gray-500 text-xs">Remember: The impostor doesn't know the word!</p>
              </motion.div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                >
                  ...
                  <p className="text-lg font-bold text-gray-800" style={{ fontFamily: 'Space Grotesk' }}>
                    ???
                  </p>
                </motion.div>
                <motion.div 
          ...
          {gameState === 'voting' && (
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl text-center h-fit">
              ...
              >
                <p className="text-xl font-bold text-gray-800 mb-1" style={{ fontFamily: 'Space Grotesk' }}>
                  {playerNames[currentVoter]}'s Turn
                </p>
                <p className="text-gray-600 text-sm">Who do you think is the impostor?</p>
              </motion.div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {playerNames.map((name, index) => (
                  <motion.button
                    ...
                    whileHover={{ scale: name !== playerNames[currentVoter] ? 1.05 : 1 }}
                    whileTap={{ scale: name !== playerNames[currentVoter] ? 0.95 : 1 }}
                  ...
                  >
                    <p className="text-lg font-bold mb-1" style={{ fontFamily: 'Space Grotesk' }}>
                      {name.toUpperCase()}
                    </p>
                    {name === playerNames[currentVoter] && (
                      <p className="text-xs text-gray-400">Can't vote yourself!</p>
                    )}
                  </motion.button>
                ))}
              </div>
          ...
          {gameState === 'results' && (
            <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-xl text-center h-fit">
              ...
              >
                <p className="text-gray-600 text-sm mb-3 font-medium">Vote Results</p>
                  ...
                  {Object.entries(getVoteCount())
                    ...
                    .map(([name, count], index) => (
                      ...
                      >
                        ...
                        <span className="text-gray-600 text-xs">{count} vote{count !== 1 ? 's' : ''}</span>
                      </motion.div>
                    ))}
                </div>
              </motion.div>
              ...
              >
                ...
                <p className="text-3xl font-bold text-blue-600 mb-2" style={{ fontFamily: 'Space Grotesk' }}>
                  {word.toUpperCase()}
                </p>
                <p className="text-gray-500 text-xs">
                  Category: {selectedCategory === 'random' ? 'Mixed' : categories[selectedCategory].name}
                </p>
              </motion.div>
              ...
              >
                ...
                <p className="text-2xl font-bold text-red-600 mb-1" style={{ fontFamily: 'Space Grotesk' }}>
